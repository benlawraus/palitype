...
def get_rst_grammar():
    return r"""// A grammar for reStructuredText language written in ANTLR v4.

start		: parse

parse		:  (element | empty_line)+

element		:  section | sectionElement

sectionElement	:  listItemBullet | listItemEnumerated | paragraph | lineBlock | comment
comment		:  Space* Comment Space* (commentLineNoBreak commentParagraphs?)?
commentParagraphs	: main=commentParagraph commentRest
commentRest	: (empty_line commentParagraph)*
commentParagraph:  commentLine+
commentLineNoBreak	:  commentLineAtoms
commentLine	:  LineBreak Space Space Space commentLineNoBreak
commentLineAtoms:  ~(LineBreak)+
"""
...
Appatiṭṭhaṃ khvāhaṃ anāyūhaṃ [#āyūhaṃ]_ ogham atari.
""",2),
    ('palitype_instr_1.yml', 'tests', "By not reaching out, is the flood crossed.\n",
...
def test_populate(filename, directory, eng, pli, eix):
    ...
    delims = list(delim_dict.keys()) #[d.token for d in delimiters]
    #print(delims)
    d = group_into_sections(delims,delimiter_locations(delims,text))
    elements = populate_sections(d, text)
    #print(elements)
    english = next(e for e in elements[eix] if delim_dict[e.token.str].tag == "English")
    pali = next(e for e in elements[eix] if delim_dict[e.token.str].tag == "Pali")
    assert english.text == eng
    assert pali.text == pli
    

@pytest.mark.parametrize('text, text_to_markup, finished_text, tok', [ 
    ("""Appatiṭṭhaṃ khvāhaṃ anāyūhaṃ 
...
d = {"token":"p=", "tag":"Pali", "inline_markup":"*",
...
def test_inline_markup(filename,directory, nr_pali):
    ...
    #print(mod_text)
    #print(f"nr modified : {mod.inline_markup}\nnr untouched : {mod.untouched}")
