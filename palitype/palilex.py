...
from .classes import Token, Delim, Mod_counter
...
def group_into_sections(delim:Tuple[str], tokens:List[Token])->List[Token]:
    """
    Takes a list of Tokens and assigns a group. Each group is 
    meant to assign to once piece of text that has a similar
    meaning. For example:: 
...
Yaml_keywords = namedtuple("Yaml_keywords", ptcon.YAML_KEYS)
...
def read_yaml_string(yaml_text:str, yk:Yaml_keywords)->TypedDict:
    ...
    schema = sy.Map({
                ...
                yk.delimiters:sy.MapPattern(sy.Str(),
                                sy.Any(), minimum_keys=2),
...
def get_markup_language(m:str)->str:
    _m = m.lower().replace(' ','')
...
def get_delim_dict(settings:TypedDict, yk:Yaml_keywords)->TypedDict:
    delim_dict = {}
    for tag,delim in settings[yk.delimiters].items():
        if tag in ['Verse']:
            _d = Delim(delim,tag.lower())
        else:
            _d = Delim(delim,tag)
        _d.inline_markup = settings.get(yk.inline_markup,{tag:''}).get(tag,'')
        _d.hide = tag in settings.get(yk.hide,[])
        _d.tooltip = tag in settings.get(yk.tooltip,[])
        _d.exclude_db = tag in settings.get(yk.exclude_db,[])
        _d.substitute = settings.get(yk.substitute,{tag:''}).get(tag,'')
        delim_dict[_d.token] = _d
    delim_dict[settings.get(yk.end_delimiter)]=\
            Delim(settings.get(yk.end_delimiter), ptcon.END_DELIMITER)
    return delim_dict


def get_settings(yaml_text:str)->List[Delim]:
    """
    Decodes the yaml string and places the information into a dict
    of class Delim.
    ...
    """
    yk = Yaml_keywords(
        markup_language = "Markup Language".lower(),
    ...
    settings = read_yaml_string(yaml_text, yk)
    m_u = get_markup_language(settings.get(yk.markup_language,"rst"))
    text_settings = {'markup_language':m_u,
                     'delim_dict':get_delim_dict(settings, yk)}

    text_settings['verse_line_markup'] = '| ' if m_u=='rst' else ''
        
    return text_settings
    
    
def markup_substitution(settings:TypedDict, 
                   delim_positions:List[Token], 
                   text:str)->str:
    """
    Takes the palitype and replace with rst equivalent.
    ...
    for _this, _next in pairwise(positions):
        _line = delim_dict[_this.str].get_modified_lines(
                             text[_this.p.end:_next.p.start],
                             mod)
        if _line is None:
            continue
        ...
        if settings['verse_line_markup']:
            if delim_dict[_next.str].tag == "verse":
                ...
                _line = [l.replace(l.strip(),verse_markup+l.strip()) for l in _line]
...
def palilex(files:List[str]):
    ...
    for infile in files:
        ...
        mod_text, mod = markup_substitution(settings,
...
def main():
    ...
    parser.add_argument('infile', metavar='in_filename', type=str, nargs='+',
